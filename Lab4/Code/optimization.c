...
void remove_redundant_label() {
    InterCodeList cur = IrList->next;
    while (cur != IrList) {
        assert(cur->code != NULL);
        if (cur->code->kind == IR_LABEL) {
            assert(cur->code->u.singleop.op != NULL);
            int label_num = cur->code->u.singleop.op->u.label_no;
            InterCodeList next = cur->next;
            while (next != IrList) {
                assert(next->code != NULL);
                if (next->code->kind != IR_LABEL) break;
                delete_ir(next);
                next->code->u.singleop.op->u.label_no = label_num;
                next = next->next;
            }
        }
        cur = cur->next;
...
BasicBlock init_bb() {
    ...
    bb->front = bb->back = NULL;
    bb->pre = init_bb_list();
    bb->next = init_bb_list();
    return bb;
}
    ...
    while (cur != IrList) {
        ...
        if (cur->code->kind == IR_IF_GOTO || cur->code->kind == IR_GOTO) {
            ...
            if (cur->code->kind == IR_GOTO) {  //跳转目标
                assert(cur->code->u.singleop.op != NULL);
                label_num = cur->code->u.singleop.op->u.label_no;
            } else {
...
void construct_bb_list() {
    ...
    construct_bb(prev_first, IrList->prev);  // 最后一个BB块
}
...
void construct_fb_array() {
    ...
    for (int i = 0; i < global_bb_count; i++) {
        ...
        assert(cur->bb->front->code != NULL);
        if (cur->bb->front->code->kind == IR_FUNC) {
            cur_fb = i;
            if (prev_fb != UNKNOWN) {
                construct_fb(fb_array[fb_number++], prev_func, prev_fb, cur_fb - 1);
            }
            prev_fb = i;
            prev_func = cur->bb->front->code->u.singleop.op->u.func_name;
        }
...
void construct_fb(FunctionBlock fb, char* func_name, int front, int back) {
    assert(front != UNKNOWN && back != UNKNOWN);
    fb->bb_first = front;
    fb->bb_last = back;
    fb->func_name = func_name;
    fb->bb_nums = back - front + 1;
}

void construct_bb(InterCodeList front, InterCodeList back) {
    assert(front && back);
    BasicBlock bb = init_bb();
    bb->pre = init_bb_list();
    bb->next = init_bb_list();  // 前继后继设为空的头结点
    bb->front = front;
    bb->back = back;
    bb->bb_no = global_bb_count++;
    InterCodeList cur = bb->front;
    do {
        cur->code->bb_no = global_bb_count - 1;  //记录所在的BB
        cur = cur->next;
    } while (cur != bb->back->next);
...
void construct_cfg() {
    for (int i = 0; i < global_bb_count; i++) {
        ...
        int label_num = UNKNOWN;
        assert(cur->bb->back->code != NULL);
        if (i == bb_first) {  // 第一块将Entry设置为其前驱
            add_bb(cur->bb->pre, entry->bb);
            add_bb(entry->bb->next, cur->bb);
        }
        if (cur->bb->back->code->kind == IR_RETURN) {  // RETURN将Exit设置为其后继
            add_bb(cur->bb->next, exit->bb);
            add_bb(exit->prev, cur->bb);
        } else if (cur->bb->back->code->kind != IR_GOTO) {
            if (i != bb_last) {
                assert(i < global_bb_count - 1);
                add_bb(cur->bb->next, bb_array[i + 1]->bb);  // 下一个BB为直接后继
        ...
        if (cur->bb->back->code->kind == IR_GOTO) {  //跳转后继
            assert(cur->bb->back->code->u.singleop.op != NULL);
            label_num = cur->bb->back->code->u.singleop.op->u.label_no;
        } else if (cur->bb->back->code->kind == IR_IF_GOTO) {
            assert(cur->bb->back->code->u.if_goto.z != NULL);
            label_num = cur->bb->back->code->u.if_goto.z->u.label_no;
        }
        if (label_num != UNKNOWN) {
            int goto_bb = label_array[label_num]->code->bb_no;
            add_bb(cur->bb->next, bb_array[goto_bb]->bb);  // 设置后继
...
void show_cfg() {
    for (int i = 0; i < global_bb_count; i++) {
        BasicBlockList cur = bb_array[i];
        show_bb_list(cur->bb->next, stdout);
...
void show_bb(BasicBlock bb, FILE* ir_out) {
    ...
    InterCodeList cur = bb->front;
    do {
        if (ir_out != stdout) show_ir(cur->code, ir_out);
        cur = cur->next;
    } while (cur != bb->back->next);
}
...
void LVA_BB_use_def(BasicBlock bb, bool* use, bool* def) {
    InterCodeList cur = bb->back;
    do {
        LVA_IR_use_def(cur->code, use, def);
        cur = cur->prev;
    } while (cur != bb->front->prev);
}
...
void LVA_IR_use_def(InterCode ir, bool* use, bool* def) {
    ...
    switch (ir->kind) {
        ...
        case IR_ARG:
        case IR_WRITE:
            set_LVA_use_def(ir->u.singleop.op, USE, use, def);
            break;
        case IR_DEC:
        ...
        case IR_PARAM:
        case IR_READ:
            set_LVA_use_def(ir->u.singleop.op, DEF, use, def);
            break;
        case IR_ASSIGN:
        case IR_ADDR:
        case IR_LOAD:
        case IR_STORE:
        case IR_CALL:
            set_LVA_use_def(ir->u.binop.left, DEF, use, def);
            set_LVA_use_def(ir->u.binop.right, USE, use, def);
            break;
        case IR_ADD:
        case IR_SUB:
        case IR_MUL:
        case IR_DIV:
            set_LVA_use_def(ir->u.tripleop.res, DEF, use, def);
            set_LVA_use_def(ir->u.tripleop.op1, USE, use, def);
            set_LVA_use_def(ir->u.tripleop.op2, USE, use, def);
            break;
        case IR_IF_GOTO:
...
void LVA_meet(int bb_first, int bb_no, bool** IN, bool** OUT) {
    ...
    BasicBlockList succ_head = bb_array[bb_no]->bb->next;
...
bool judge_IR_DA(InterCode ir, bool* out) {
    ...
    switch (ir->kind) {
        ...
        case IR_ARG:
        case IR_WRITE:
            set_bitset(ir->u.singleop.op, out, true);
            break;
        case IR_DEC:
        ...
        case IR_READ:  // READ PARAM不受控
            set_bitset(ir->u.singleop.op, out, false);
            break;
        case IR_ASSIGN:
        case IR_ADDR:
        case IR_LOAD:
        case IR_STORE:
            res_id = get_variable_id(ir->u.binop.left);
            if (res_id != -1 && out[res_id] == 0) {
                result = true;
            } else {
                set_bitset(ir->u.binop.left, out, false);
                set_bitset(ir->u.binop.right, out, true);
            }
            break;
        case IR_CALL:
        ...
        case IR_MUL:
        case IR_DIV:
            res_id = get_variable_id(ir->u.tripleop.res);
            if (res_id != -1 && out[res_id] == 0) {
                result = true;
            } else {
                set_bitset(ir->u.tripleop.res, out, false);
                set_bitset(ir->u.tripleop.op1, out, true);
                set_bitset(ir->u.tripleop.op2, out, true);
            }
            break;
        case IR_IF_GOTO:
...
void CP_IR_out(InterCode ir, CPPair* out) {
    ...
    switch (ir->kind) {
        ...
        case IR_PARAM:
        case IR_READ:
            set_CP_out(ir->u.singleop.op, NAC, UNKNOWN, out);
            break;
        case IR_ASSIGN:
        case IR_CALL: {
            int left_id = get_variable_id(ir->u.binop.left);
            int right_id = get_variable_id(ir->u.binop.right);
            int right_kind = ir->u.binop.right->kind;
            if (left_id != -1) {
                if (right_id != -1) {
                    if (out[right_id].kind == CONST) {
                        set_CP_out(ir->u.binop.left, CONST, out[right_id].val, out);
                    } else if (out[right_id].kind == NAC) {
                        set_CP_out(ir->u.binop.left, NAC, UNKNOWN, out);
                    } else {
                        set_CP_out(ir->u.binop.left, UNDEF, UNKNOWN, out);
                    }
                } else if (right_kind == OP_CONSTANT) {
                    set_CP_out(ir->u.binop.left, CONST, ir->u.binop.right->u.const_val, out);
                } else {
                    set_CP_out(ir->u.binop.left, NAC, UNKNOWN, out);
                }
            }
        ...
        case IR_MUL:
        case IR_DIV: {
            int res_id = get_variable_id(ir->u.tripleop.res);
            int op1_id = get_variable_id(ir->u.tripleop.op1);
            int op2_id = get_variable_id(ir->u.tripleop.op2);
            int op1_kind = ir->u.tripleop.op1->kind;
            int op2_kind = ir->u.tripleop.op2->kind;
            if (res_id != -1) {
                if (op1_id != -1 && op2_id != -1) {  //操作数均为变量
                    if (out[op1_id].kind == CONST && out[op2_id].kind == CONST) {
                        ...
                        set_CP_out(ir->u.tripleop.res, CONST, val, out);
                    } else if (out[op1_id].kind == NAC || out[op2_id].kind == NAC) {
                        set_CP_out(ir->u.tripleop.res, NAC, UNKNOWN, out);
                    } else {
                        set_CP_out(ir->u.tripleop.res, UNDEF, UNKNOWN, out);
                    }
                } else if (op1_id != -1 && op2_kind == OP_CONSTANT) {
                    if (out[op1_id].kind == CONST) {
                        long long val;
                        long long op1_val = out[op1_id].val, op2_val = ir->u.tripleop.op2->u.const_val;
                        if (ir->kind == IR_ADD) {
                            val = op1_val + op2_val;
                        } else if (ir->kind == IR_SUB) {
                            val = op1_val - op2_val;
                    ...
                    if (out[op2_id].kind == CONST) {
                        long long val;
                        long long op1_val = ir->u.tripleop.op1->u.const_val, op2_val = out[op2_id].val;
                        if (ir->kind == IR_ADD) {
                            val = op1_val + op2_val;
                        } else if (ir->kind == IR_SUB) {
                            val = op1_val - op2_val;
                    ...
                    long long val;
                    long long op1_val = ir->u.tripleop.op1->u.const_val, op2_val = ir->u.tripleop.op2->u.const_val;
                    if (ir->kind == IR_ADD) {
                        val = op1_val + op2_val;
                    } else if (ir->kind == IR_SUB) {
                        val = op1_val - op2_val;
...
void CP_IR_constant(InterCodeList ir_point, CPPair* out) {
    ...
    switch (ir->kind) {
        ...
        case IR_ARG:
        case IR_WRITE: {
            int id = get_variable_id(ir->u.singleop.op);
            if (id != -1 && out[id].kind == CONST) {
                ir->u.singleop.op = gen_operand(OP_CONSTANT, out[id].val, -1, NULL);
            }
            break;
        }
        case IR_PARAM:
            ...
            if (left_id != -1) {
                if (right_id != -1) {
                    if (out[right_id].kind == CONST) {
                        ...
                        ir_point->code = gen_ir(NULL, IR_ASSIGN, ir->u.binop.left, right_val, NULL, -1, NULL);
                    } else if (out[right_id].kind == NAC) {
                        set_CP_out(ir->u.binop.left, NAC, UNKNOWN, out);
                    } else {
                        set_CP_out(ir->u.binop.left, UNDEF, UNKNOWN, out);
                    ...
                    set_CP_out(ir->u.binop.left, CONST, ir->u.binop.right->u.const_val, out);
                    Operand right_val = gen_operand(OP_CONSTANT, ir->u.binop.right->u.const_val, -1, NULL);
                    ir_point->code = gen_ir(NULL, IR_ASSIGN, ir->u.binop.left, right_val, NULL, -1, NULL);
                } else {
                    set_CP_out(ir->u.binop.left, NAC, UNKNOWN, out);
                }
        ...
        case IR_DIV: {
            ...
            if (op1_id != -1 && out[op1_id].kind == CONST) {
                ...
                ir->u.tripleop.op1 = gen_operand(OP_CONSTANT, op1_val, -1, NULL);
            }
            if (op2_id != -1 && out[op2_id].kind == CONST) {
                long long op2_val = out[op2_id].val;
                ir->u.tripleop.op2 = gen_operand(OP_CONSTANT, op2_val, -1, NULL);
            }
            if (res_id != -1) {
                if (op1_id != -1 && op2_id != -1) {  //操作数均为变量
                    if (out[op1_id].kind == CONST && out[op2_id].kind == CONST) {
                        ...
                        ir_point->code = gen_ir(NULL, IR_ASSIGN, ir->u.tripleop.res, right_val, NULL, -1, NULL);
                    } else if (out[op1_id].kind == NAC || out[op2_id].kind == NAC) {
                        set_CP_out(ir->u.tripleop.res, NAC, UNKNOWN, out);
                    } else {
                        set_CP_out(ir->u.tripleop.res, UNDEF, UNKNOWN, out);
