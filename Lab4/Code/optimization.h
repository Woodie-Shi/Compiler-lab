...
extern int global_bb_count;       // BB块个数
extern int global_fb_count;       // FB块个数
extern int fb_number;             // FB块编号
extern int var_nums;              // 变量加临时变量个数
extern BasicBlockList* bb_array;  // BB链表数组，便于索引
extern FunctionBlock* fb_array;   // FB数组，便于索引
...
struct BasicBlock_ {
    int fb_no;                  // 所处的FB块编号
    int bb_no;                  // BB块编号
    BasicBlockList pre, next;    // 前驱，后继BB链表
    InterCodeList front, back;  // BB的首指令和尾指令
};
...
void construct_fb(FunctionBlock fb, char* func_name, int front, int back);  // 创建FB
void construct_bb(InterCodeList front, InterCodeList back);                 // 创建BB
