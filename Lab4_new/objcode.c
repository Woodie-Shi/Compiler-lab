...
void generateObjectCode(FILE *file)
{
    ...
    InterCodeList curInterCodes = IrList->next;
    while (curInterCodes != IrList) // 遍历中间代码，生成目标代码
    {
        ...
        case IR_ARG:
        {
            ...
            while (curInterCodes->code->kind != IR_CALL)
    ...
    if (op->kind == OP_CONSTANT)
    {
        for (int i = 8; i <= 15; i++)
        {
            if (Registers[i].used == 0)
            {
                Registers[i].used = 1;
                Registers[i].variable = NULL;
                fprintf(file, "\tli %s, %d\n", Registers[i].name, op->u.const_val);
                return i;
            }
        }
    }
...
/* 目标代码生成：label */
void generateLableObjectCode(InterCodeList curInterCodes, FILE *file)
{
    fprintf(file, "label%d:\n", curInterCodes->code.u.singleOP.op->var_no);
}

/* 目标代码生成：func */
void generateFuncObjectCode(InterCodeList curInterCodes, FILE *file)
{
    ...
    InterCodeList tmpInterCodes = curInterCodes;
...
/* 目标代码生成：assign */
void generateAssignObjectCode(InterCodeList curInterCodes, FILE *file)
{
...
/* 目标代码生成：add */
void generateAddObjectCode(InterCodeList curInterCodes, FILE *file)
{
...
/* 目标代码生成：sub */
void generateSubObjectCode(InterCodeList curInterCodes, FILE *file)
{
    int resultIndex = findRegisters(curInterCodes->code.u.tripleOP.result, file);
    int op1Index = findRegisters(curInterCodes->code.u.tripleOP.op1, file);
    int op2Index = findRegisters(curInterCodes->code.u.tripleOP.op2, file);
...
/* 目标代码生成：mul */
void generateMulObjectCode(InterCodeList curInterCodes, FILE *file)
{
    int resultIndex = findRegisters(curInterCodes->code.u.tripleOP.result, file);
    int op1Index = findRegisters(curInterCodes->code.u.tripleOP.op1, file);
    int op2Index = findRegisters(curInterCodes->code.u.tripleOP.op2, file);
...
/* 目标代码生成：div */
void generateDivObjectCode(InterCodeList curInterCodes, FILE *file)
{
    int resultIndex = findRegisters(curInterCodes->code.u.tripleOP.result, file);
    int op1Index = findRegisters(curInterCodes->code.u.tripleOP.op1, file);
    int op2Index = findRegisters(curInterCodes->code.u.tripleOP.op2, file);
...
// 中间代码生成时未产生此下三种类型
void generateGetAddrObjectCode(InterCodeList curInterCodes, FILE *file) {}

void generateGetContentObjectCode(InterCodeList curInterCodes, FILE *file) {}

void generateToAddrObjectCode(InterCodeList curInterCodes, FILE *file) {}

/* 目标代码生成：goto */
void generateGotoObjectCode(InterCodeList curInterCodes, FILE *file)
{
    fprintf(file, "\tj label%d\n", curInterCodes->code.u.singleOP.op->var_no);
}

/* 目标代码生成：ifgoto */
void generateIfgotoObjectCode(InterCodeList curInterCodes, FILE *file)
{
...
/* 目标代码生成：return */
void generateReturnObjectCode(InterCodeList curInterCodes, FILE *file)
{
...
// func中已经处理了dec内容
void generateDecObjectCode(InterCodeList curInterCodes, FILE *file) {}

/* 目标代码生成：arg */
void generateArgObjectCode(InterCodeList curInterCodes, FILE *file)
{
    int argCount = 0;
...
// arg后整体处理
void generateCallObjectCode(InterCodeList curInterCodes, FILE *file)
{
}

// func中处理
void generateParamObjectCode(InterCodeList curInterCodes, FILE *file)
{
}

/* 目标代码生成：read */
void generateReadObjectCode(InterCodeList curInterCodes, FILE *file)
{
    fprintf(file, "\taddi $sp, $sp, -4\n");
...
/* 目标代码生成：write */
void generateWriteObjectCode(InterCodeList curInterCodes, FILE *file)
{
    int index = findRegisters(curInterCodes->code.u.singleOP.op, file);
